 ...
 * - IPv6 bracketed or raw addresses
 */
function parseProxyString(proxyStr) {
    ...
    // Handle colon-separated formats (host:port:user:pass)
    if (str.startsWith('[')) {
...
function createBot(username, host, port, password, proxyUrl, reconnectDelay, socket) {
    ...
    if (parsedProxy) {
        botOptions.connect = (client) => {
            socket.emit('bot_log', { 
                botId: username, 
                text: `Opening SOCKS5 Tunnel to ${host}:${port}...` 
            });
            ...
            const socksOptions = {
                ...
                command: 'connect',
                destination: {
                    host: host, // Pass exact host domain string to avoid local IPv4 DNS failure
                    port: port
                }
            };
